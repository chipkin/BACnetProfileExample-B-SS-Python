#
# CAS BACnet Stack Python Adapter - HELPERS ONLY, NOT A WORKING ADAPTER
#
# CR-187 (batch 116-A, sprint 208, #743): despite the name and the description below, this file
# never loads the native library and never calls any BACnetStack_* export. It defines only
# platform detection (the libname logic below) and ctypes.CFUNCTYPE callback-signature
# constants - useful references if you are writing your own Python binding, but not, by
# themselves, something that talks to the stack. A real Python adapter (ctypes.CDLL/WinDLL
# loading, argtypes-bound wrapper functions) is tracked as a separate feature request; this file
# is not it.
#
...
# Callbacks ---------------------------------------------------------------------------
#
# The ctypes.CFUNCTYPE callback types, and the argtypes/restype bindings for every export,
# are GENERATED into CASBACnetStackAdapterBindings.py by
# ci_scripts/generate-python-adapter.js:
#
#     node ci_scripts/generate-python-adapter.js
#
# They are named for the C typedefs (FPCallbackReceiveMessage, ...), the same names the C#,
# Java and Rust adapters use. Use them from there:
#
#     import ctypes
...
#
# The 39 hand-written `fpCallback*` / `fpHook*` CFUNCTYPE constants that used to sit here were
# removed when generation landed: they covered 39 of the 69 callbacks, carried no argtypes for
# any export, and had no gate comparing any of them against the header.
#
# What stays in THIS file, hand-written: the platform detection above and the BACnet
# enumeration dictionaries below.
